...
{    
    ...
    // script_name строка запроса без параметров
    ...
    uri_len = (uri_len >= BTP_STR_BUFFER_SIZE) ? BTP_STR_BUFFER_SIZE - 1 : uri_len;
    script_name = (char *)r->unparsed_uri.data;    

    memset(result, '\0', BTP_STR_BUFFER_SIZE);           
    
    if (status == 200) {            
        memcpy(result, "unknown_ok", sizeof("unknown_ok"));
    }
    else if (r->unparsed_uri.len == 0) {
        memcpy(result, "empty_url", sizeof("empty_url"));
    }
    else if (strcmp("/", script_name) == 0) {
        memcpy(result, "/",  sizeof("/"));
    }
    else if (
        strstr(script_name, ".phtml") != 0
        || strstr(script_name, ".php") != 0  
    ) {
        memcpy(result, script_name, uri_len);
    }
    else {
